#!/bin/bash
# Entrypoint for the ZKFC controller that watches namenode2 (nn2).
#
...
# container. Here it runs in its own container (hostname namenode2-zkfc,
# not namenode2), so auto-detection fails with:
...
sed -i "s|</configuration>|<property><name>dfs.ha.namenode.id</name><value>nn2</value></property></configuration>|" \
